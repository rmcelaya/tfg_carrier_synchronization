function [SER,BER] = first_order_costas_decision(EbN0,fc,Rb,plotear,save_data,N,doppler,offset_phase,variation,initial_phase,gamma)
%% Docs
...
for k=1:N_s
    ...
    %psi = psi +gamma*(1+ro)*err-gamma*prev_err;
    psi = err*gamma;
...
if plotear == true
    ...
    fprintf("SER = %d\n",SER);
    fprintf("BER = %d\n",BER);
    fprintf("Phase mean = %d\n",mean(phase_o));
    fprintf("Phase std = %d\n",sqrt(var(phase_o,1)));
    figure;
    plot((phase_o));
    title("Phase estimate over time");
    xlabel("# of sent bits");
    ylabel("$\frac{Phase}{\pi}$ [rad]",'Interpreter','Latex');

    hold on;
    grid on;
    plot(ones(1,N_s+2)*offset_phase);
    legend("Estimated phase","True phase");
end

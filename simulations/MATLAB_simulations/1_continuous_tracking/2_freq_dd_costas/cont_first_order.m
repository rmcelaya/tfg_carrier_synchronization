...
function [SER,BER] = cont_first_order(EbN0,fc,Rb,plotear,save_data,N,doppler,offset_phase,variation,initial_freq,gamma,alpha)
%% Docs
...
if plotear == true
    ...
    figure;
    plot((freq-doppler)/Rb);
    title("Frequency estimate over time");
    xlabel("# of sent symbols");
    ylabel("Relative frequency error (with respect to Rb)");
    hold on;
    grid on;
    plot(ones(1,N_s+2)*(doppler-doppler)/Rb);
